...
import { Database, Loader2, Mail, Lock, User, ArrowRight, Eye, EyeOff } from "lucide-react";
import { registerUser, setTokens, getAccessToken, getCurrentUser } from "@/app/lib/auth";
...
export default function RegisterPage() {
  ...
  async function handleRegister(e: FormEvent) {
    ...
    try {
      const data = await registerUser(email.trim(), fullName.trim(), password);
        ...
        <div className="relative bg-white/80 dark:bg-[#1a1a2e]/90 backdrop-blur-xl border border-white/50 dark:border-slate-800/60 rounded-3xl shadow-2xl shadow-blue-500/5 dark:shadow-blue-500/5 p-8 md:p-10">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5">
              Join SmartHub and start building your AI workspace
            </p>
          </div>

          {/* Error Alert */}
          {error && (
            <div className="mb-6 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 px-4 py-3 text-sm text-red-700 dark:text-red-400">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            {/* Full Name */}
            <div>
              <label htmlFor="register-name" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                Full Name
              </label>
            ...
            {/* Email */}
            <div>
              <label htmlFor="register-email" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                Email address
              </label>
              <div className="relative">
            ...
            {/* Password */}
            <div>
              <label htmlFor="register-password" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                Password
              </label>
              <div className="relative">
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="register-confirm" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                Confirm Password
              </label>
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4.5 w-4.5 text-slate-400 dark:text-slate-500" />
            ...
            <button
              type="submit"
              disabled={isLoading || !email.trim() || !fullName.trim() || !password || !confirmPassword}
