...
def migrate():
    try:
        ...
        # Read user_id this time!
        cursor.execute("SELECT id, content, metadata, embedding, user_id FROM documents WHERE embedding IS NOT NULL")
        rows = cursor.fetchall()
        
        if len(rows) > 0:
            collection = get_chroma_collection()
            
            # Clear old collection to avoid duplicates
            import chromadb
            ...
            print(f"Migration complete! {len(embeddings)} chunks added with user_id.")
